...
def _get_client(context: ContextTypes.DEFAULT_TYPE) -> DeepSeekClientInterface:
    client = context.chat_data.get("client")
    if client is None:
        client = DeepSeekClient(context.bot_data["store"])
        context.chat_data["client"] = client
    return client


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Wiping chat_data alone would drop the client but leave its stored history,
    # which the next client would load straight back in.
    _get_client(context).clear_history()
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    replies = await _get_client(context).get_broadcast_replies(prompt, system_prompt)
...
async def broadcast_and_compare_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    system_prompt = context.chat_data.get("system_prompt", DeepSeekClient.DEFAULT_SYSTEM_PROMPT)
    await update.message.chat.send_action("typing")
    replies, comparisons = await _get_client(context).get_broadcast_and_compare(prompt, system_prompt)
...
def _stats_text(context: ContextTypes.DEFAULT_TYPE) -> str:
    client = context.chat_data.get("client")
    history = client.history if client else []
