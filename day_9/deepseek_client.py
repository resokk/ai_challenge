...
from storage import MessageStore, count_tokens
...
class CompressionResult(NamedTuple):
    ...
    folded: int  # messages replaced by the summary
    tokens_before: int  # tokens the dialog history held before
    tokens_after: int  # tokens it holds now, summary included
...
class DeepSeekClientInterface(ABC):
    ...
    @abstractmethod
    async def compress_history(self, threshold: int) -> CompressionResult | None: ...
...
class DeepSeekClient(DeepSeekClientInterface):
    ...
    # Summaries are a fidelity job, not a creative one, so they ignore the
    # chat's temperature and always run cold.
    ...
    def __init__(self, store: MessageStore) -> None:
        self._store = store
        self._history: list[dict] = store.load_history(self.MAX_HISTORY_MESSAGES)
    ...
    def add_assistant_message(self, content: str) -> None:
        self._store.record("assistant", content)
        self.history.append({"role": "assistant", "content": content})

    def discard_last_user_message(self) -> None:
        if self.history and self.history[-1]["role"] == "user":
            self.history.pop()
            self._store.discard_last_incoming()
    ...
    async def compress_history(self, threshold: int) -> CompressionResult | None:
        """Once the history has grown to `threshold` messages, summarize its
        oldest half and put the summary in their place. Returns what the round
        cost and saved, or None if nothing was compressed.

        This shrinks the context window, not the record: the stored log keeps
        every message, so compression never destroys anything the user said.
        The summary is saved beside the log, so a restart resumes from the
        compressed conversation rather than re-reading the raw one.

        Compression is maintenance, not part of answering the user, so a failed
        summary is logged and skipped rather than raised: the history simply
        stays uncompressed until the next message."""
        if not threshold or len(self.history) < threshold:
            return None
        ...
        if not summary:
            logger.warning("Summarization returned an empty summary, leaving history uncompressed")
            return None
        ...
        self._store.record_summary(summary_message["content"], keep_last=len(kept))
        return CompressionResult(half, tokens_before, self._history_tokens())
