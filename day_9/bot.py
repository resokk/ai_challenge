...
def _get_client(update: Update, context: ContextTypes.DEFAULT_TYPE) -> DeepSeekClientInterface:
    ...
    if client is None:
        client = clients[chat_id] = DeepSeekClient(context.bot_data["store"])
    return client
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        f"/compression [{'|'.join(COMPRESSION_OPTIONS)}] - view or set history compression "
        "(0 is off; otherwise the oldest half of the history is summarized once it reaches that "
        "many messages)\n"
...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # After the answer is out, so summarizing never delays the user's reply.
    compression = context.chat_data.get("compression", DeepSeekClient.DEFAULT_COMPRESSION)
    compressed = await client.compress_history(compression)
    if compressed:
        await update.message.reply_text(
            f"Compressed the {compressed.folded} oldest messages into a summary "
            f"({len(client.history)} messages now in context).\n"
            f"History tokens: {compressed.tokens_before} before, {compressed.tokens_after} after."
        )
