...
TASK_STATE_SEQUENCE = ("PENDING", "PLAN", "EXECUTE", "REVIEW", "DONE")
...
TASK_STATES = (*TASK_STATE_SEQUENCE, CANCELED_STATE)
...
def allowed_states(state: str) -> tuple[str, ...]:
    """Where a task in `state` may move. The run is walked a step at a time -
    on to the next state, or back to the one before, to correct a move or to
    reopen work that came back - and a task on the run can always be canceled.
    A canceled task has stopped rather than advanced, so the only way out of it
    is to start the run again from the beginning.

    This is the whole of the transition rule, and every move goes through it:
    what the user types at /state and what the assistant asks for with set_state
    are checked against the same tuple."""
    if state not in TASK_STATE_SEQUENCE:
        return (DEFAULT_TASK_STATE,)
    at = TASK_STATE_SEQUENCE.index(state)
    previous = TASK_STATE_SEQUENCE[max(at - 1, 0):at]
    following = TASK_STATE_SEQUENCE[at + 1:at + 2]
    return (*previous, *following, CANCELED_STATE)
...
class UserStore:
    ...
    def save_state(self, user_id: int, task_id: str, state: str) -> None:
        """Move a task to `state`, which the caller has checked against
        allowed_states - the run is the bot's rule, not the database's."""
